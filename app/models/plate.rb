...
class Plate < ApplicationRecord
  ...
  enum state: %i[p_preparing p_prepared p_testing p_complete p_analysed]
  ...
  scope :is_preparing, -> { where(state: Plate.states[:p_preparing]) }
  scope :is_prepared, -> { where(state: Plate.states[:p_prepared]) }
  scope :is_testing, -> { where(state: Plate.states[:p_testing]) }
  scope :is_complete, -> { where(state: Plate.states[:p_complete]) }
  scope :is_done, -> { where(state: Plate.states[:p_analysed]) }
