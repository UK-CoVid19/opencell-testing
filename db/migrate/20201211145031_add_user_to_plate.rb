class AddUserToPlate < ActiveRecord::Migration[6.0]
  def up
    ...
    dummy_user = !User.all.any? ? User.create(email: 'dummy@user.com', password: pwd, password_confirmation: pwd) : User.first
    Plate.where(user_id: nil).update_all(user_id: dummy_user.id)
